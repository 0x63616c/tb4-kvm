...
| 40 Gb/s signal integrity | **Not validated for proposed hardware** | Retained current-desk topology capture observes the OWC dock in USB4 mode at 40 Gb/s; validation ladder exists | Simulated and measured prototype channel, repeated functional traffic matrix, pre-compliance and lab results |
...
| Product validation | **Plan plus partial topology evidence** | `docs/VALIDATION-PLAN.md`; sanitized literal current-dock topology capture retained | Exact host model, cable and full desk-behavior baseline context plus Rev A/Rev B hardware results, logs, fixtures, pass/fail thresholds and certification/interoperability evidence |
